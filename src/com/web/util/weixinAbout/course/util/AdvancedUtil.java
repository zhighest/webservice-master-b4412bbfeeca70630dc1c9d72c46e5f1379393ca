package com.web.util.weixinAbout.course.util;
...
import com.web.util.weixinAbout.course.message.resp.Article;
import com.web.util.weixinAbout.course.message.resp.Music;
import com.web.util.weixinAbout.course.pojo.SNSUserInfo;
import com.web.util.weixinAbout.course.pojo.WeixinGroup;
import com.web.util.weixinAbout.course.pojo.WeixinMedia;
import com.web.util.weixinAbout.course.pojo.WeixinOauth2Token;
import com.web.util.weixinAbout.course.pojo.WeixinQRCode;
import com.web.util.weixinAbout.course.pojo.WeixinUserInfo;
import com.web.util.weixinAbout.course.pojo.WeixinUserList;
...
public class AdvancedUtil {
	 ...
	 */
	public static String getOauth2Code(String appid, String redirctUrl) {
		WeixinOauth2Token wat = null;
		// 拼接请求地址
		String requestUrl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + appid +
				"&redirect_uri="+redirctUrl+"&response_type=code&scope=snsapi_base&state=1#wechat_redirect";
		// 获取网页授权凭证
		log.debug("******************************************授权凭证地址获取："+requestUrl);
		String code ="";
		...
		if (null != jsonObject) {
			try {
				...
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				log.error("获取CODE失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return code;
	}
	...
	public static Map  getSHAoneURL(String timestamp, String url, String noncestr,String appid,String appServlet) {
		// TODO Auto-generated method stub
		String requestUrl = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid="+appid+"&secret="+appServlet;
		String accessToken ="";
		log.debug("******************************************授权requestUrl："+requestUrl);
		JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "GET", null);
		if (null != jsonObject) {
			try {
				accessToken = String.valueOf(jsonObject.getString("access_token"));
			} catch (Exception e) {
				requestUrl = "";
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
		...
		log.debug("******************************************授权TOKKENENENENENENEN："+accessToken);
		// 拼接请求地址   
		String requestUrlz = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token=ACCESSTOKEN&type=jsapi";
		requestUrlz = requestUrlz.replace("ACCESSTOKEN", accessToken);
		
		log.debug("******************************************授权requestUrlz："+requestUrlz);
		JSONObject jsonObjectz = CommonUtil.httpsRequest(requestUrlz, "GET", null);
		String shaURL ="";
		if (null != jsonObjectz) {
			try {
				shaURL = "jsapi_ticket="+String.valueOf(jsonObjectz.getString("ticket"))+
								                "&noncestr="+noncestr+"&timestamp="+timestamp+"&url="+url;
			} catch (Exception e) {
				requestUrlz = "";
				int errorCode = jsonObjectz.getInt("errcode");
				String errorMsg = jsonObjectz.getString("errmsg");
				log.error("获取网页授权凭证失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		Map map = new HashMap();
		map.put("ticket", String.valueOf(jsonObjectz.getString("ticket")));
		map.put("shaURL", shaURL);
