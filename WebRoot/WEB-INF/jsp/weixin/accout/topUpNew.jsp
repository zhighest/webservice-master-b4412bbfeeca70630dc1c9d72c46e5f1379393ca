...
<body style="position:fixed">
			...
			<div class="hasTBBorder whiteBkg MB10" id="hasCard" style="display:none;">
				<div class="whiteBkg width90 blockC PLR5P clearfix PTB10">
					...
					<div class="fr width80 textL">
						...
						<p class="grayTex font14">限额:<span class="blackTex" id="limitMoney">单笔 ${bankLimit } 当日 ${bankDayLimit }</span></p>	
					</div>
				</div>	
			</div>
			<!-- 已绑定银行卡银行信息 end -->
				...
				<div class="hasTBBorder whiteBkg MB10" id="noHasCard">
					<div class="whiteBkg width90 blockC PLR5P clearfix PTB15">
						...
						<c:choose>
							...
							<c:otherwise>
								<input type="text" placeholder="请输入银行卡号" id="bankAccountNo" name="bankAccountNo" class="width75 inputNoborder fr">
							</c:otherwise>
						</c:choose>
					</div>	
				</div>
    ...
    <input type="text" id="bankCardNumber" value="${bankCardNumber}" />
    <!-- <input type="text" id="bankName" value="${bankName}" /> -->
    <input type="text" id="acctBalance" value="${acctBalance}" />
    <input type="text" id="payFlag"	value="${payFlag}" />
