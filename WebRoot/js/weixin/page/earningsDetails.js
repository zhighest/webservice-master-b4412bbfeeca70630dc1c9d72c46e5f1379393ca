...
// 历史本金
var getInvFlowList = function(page) { 
    ajaxRequest(contextPath + "wxtrade/historyAmount", "current="+"&userId=" + page + "&pageSize=30", setInvFlowList,"GET");
}
var setInvFlowList = function(data) {
    $("#totalInvestAmount").html(numFormat(data.totalInvestAmount));
    var html = ""; //拼接html
    var arr = new Array;
    ...
    if (data.list == null || data.list == "") {
        ...
        html += '<h4 class="p1">暂无历史本金</h4>';
        html += '<p class="p2">如果有历史本金，您会在这里看到</p>';
        html += '</div>';
    } else {
            $.each(data.list,function(i, jsonObj) {
             arr.push(jsonObj.investAmount);
          });
            max = Math.max.apply(null, arr);
            min = Math.min.apply(null, arr); 
            $.each(data.list,function(i, jsonObj) {
              if (jsonObj.investAmount == "0.00"){

              }
              else {
                  html += '<li class="MB10" onclick="rotate($(this))">';
                  html += '<div class="width100 whiteBkg clearfix positionR list">';
                  if(jsonObj.investAmount/max*100 > 50){
                       if(i == 0){
                          html += '<div class="blockC bkg3 fl positionA rotate1" style="width:'+ jsonObj.investAmount/max*100 +'%">';
                      }else{
                          html += '<div class="blockC bkg2 fl positionA rotate1" style="width:'+ jsonObj.investAmount/max*100 +'%">';
                      }
                  }
                  else {
                     if(i == 0){    
                  ...
                  html += '</div><div class="fr font14 MR10 whiteTex">' + numFormat(jsonObj.investAmount) + '</div>';
                  html += '</div>';
                  if(jsonObj.investAmount/max*100 > 50){
                    html += '<div class="blockC bkg3 fl positionA rotate2" style="width:'+ jsonObj.investAmount/max*100 +'%">';
                  }else{ 
                    html += '<div class="blockC bkg3 fl positionA rotate2" style="width:50%">';
                  }
                  html += '<div class="fl ML10">';
                  html += '<h4 class="font14 whiteTex">'+numFormat(jsonObj.investAmount)+'</h4>';
    ...
    //console.log(min, max);
    $("#investAmount").append(html);//将拼接内容放入id为fundFlowList的ul中
};

// 历史收益
var getFundFlowList = function(page) { 
    ajaxRequest(contextPath + "wxtrade/historyAmount", "current="+"&userId=" + page + "&pageSize=30", setFundFlowList,"GET");
}
var setFundFlowList = function(data) {
    ...
    if (data.list == null || data.list == "") {
        ...
        html += '<p class="p2">如果有历史收益，您会在这里看到</p>';
        html += '</div>';
    } else {
        $.each(data.list,function(i, jsonObj) {
...
var getTotalFlowList = function(page) { 
    ajaxRequest(contextPath + "wxtrade/historyAmount", "current="+"&userId=" + page + "&pageSize=30", setTotalFlowList,"GET");  
}
var setTotalFlowList  = function(data) {
    ...
    if (data.list == null || data.list == "") {
        ...
        html += '<p class="p2">如果有年化收益率，您会在这里看到</p>';
        html += '</div>';
    } else {
        $.each(data.list,function(i, jsonObj) {
...
//加息收益率
var getAddFlowList = function(page) { 
    ajaxRequest(contextPath + "wxtrade/historyAmount", "current="+"&userId=" + page + "&pageSize=30", setAddFlowList,"GET");  
}
var setAddFlowList  = function(data) {
    var html = ""; //拼接html
    var arr = new Array;
    var min=arr[0], max=arr[0];
    if (data.list == null || data.list == "") {
        html += '<div class="listNull">';
        html += '<img src="../pic/weixin/list.png">';
        html += '<h4 class="p1">暂无加息收益率</h4>';
        html += '<p class="p2">如果有加息收益率，您会在这里看到</p>';
        html += '</div>';
    } else {
         $.each(data.list,function(i, jsonObj) {
         arr.push(parseFloat(jsonObj.addrateRise));
        });
        var sum = 0;
        for (var i = arr.length - 1; i >= 0; i--) {
             sum += arr[i]
        ...
        if (parseFloat(sum) == 0){
          ...
          $.each(data.list,function(i, jsonObj) {
            ...
            if (jsonObj.addrateRise == "0.00"){
                }
            else {
              html += '<li class="MB10" onclick="rotate($(this))">';
              html += '<div class="width100 whiteBkg clearfix positionR list">';
              if(jsonObj.addrateRise/max*100 > 50){
                  if(i == 0){
                      html += '<div class="blockC bkg3 fl positionA rotate1" style="width:'+ jsonObj.addrateRise/max*100 +'%">';
                  }else{
                      html += '<div class="blockC bkg2 fl positionA rotate1" style="width:'+ jsonObj.addrateRise/max*100 +'%">';
                  }
              }else{
                  if(i == 0){    
                      html += '<div class="blockC bkg3 fl positionA rotate1" style="width:50%">';
              ...
              html += '<div class="fl ML10">';
              html += '<h4 class="font14 whiteTex">' + jsonObj.incomeDay + '</h4>';
              html += '</div><div class="fr font14 MR10 whiteTex">' + numFormat(jsonObj.addrateRise) + '</div>';
              html += '</div>';
              if(jsonObj.addrateRise/max*100 > 50){
                  html += '<div class="blockC bkg3 fl positionA rotate2" style="width:'+ jsonObj.addrateRise/max*100 +'%">';
              }else{ 
                  html += '<div class="blockC bkg3 fl positionA rotate2" style="width:50%">';
              }
              html += '<div class="fl ML10">';
              html += '<h4 class="font14 whiteTex">'+numFormat(jsonObj.addrateRise)+'</h4>';
              html += '</div><div class="fr font14 MR10 whiteTex">'+jsonObj.incomeDay+'</div>';
              html += '</div></div>';
              html += '</li>';
            }
    ...
    //console.log(min, max);
  $("#addFlowList").append(html);//将拼接内容放入id为fundFlowList的ul中
};
...
$(document).ready(function() {
  ...
  }else if(tabSkip == 'investAmount'){
    tabSwitchLi('tabNav','tab3');tabSwitchCon('tabContent','tabContent3');tab3Click()
  }
    getTotalFlowList(1);
    // getAddFlowList(1);
    // getFundFlowList(1);
