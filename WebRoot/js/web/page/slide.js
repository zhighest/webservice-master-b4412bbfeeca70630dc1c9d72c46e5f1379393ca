	...
	$.fn.slide = function(options) {
		...
		$.fn.moveLeft = function() {
			...
			if (index <= total && index > 1) {
				...
				next1 = $(settings.slideContainer).eq(index - 2);
				if (next1) {
		...
		$.fn.moveRight = function() {
			...
			if (index >= 1 && index < total) {
				current = $(settings.slideContainer).eq(index - 1);
				next1 = $(settings.slideContainer).eq(index);
				if (next1) {
					current.removeClass("active");
					next1.addClass("active");
					changeNavList();
				};
				pos = ((Number(next1.attr("data-index")) - 1) * 100) * -1;
				el.changeSlide(settings, pos);
		...
		$("<ul style='width:" + ($(settings.slideContainer).length * 14 + ($(settings.slideContainer).length) * 20) + "px;margin-left:-" + ($(settings.slideContainer).length * 14 + (($(settings.slideContainer).length) * 20)) / 2 + "px' class='navListBox'>" + navlist + "</ul>").appendTo(el.parent());
...
function bannerlistener(){var i=0,a=0,missabb=setInterval(function(){"0"==$("#banner3").offset().left?(i+=2,a++,a>10&&15>a?$("#bannertitle").css({width:100+i+"%",top:-i/2+"%",left:-i/2+"%"}):a>16&&$("#bannertitle").css({width:"100%",top:"0",left:"0"})):($("#bannertitle").css("width","100%"),a=0,i=0)},100);}
